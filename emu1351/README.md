...
| GPIO 2 | MODE_PROPORTIONAL | Mode select: Proportional mode (input with pull-up) |
| GPIO 3 | MODE_JOYSTICK | Mode select: Joystick mode (input with pull-up) |
| GPIO 4 | LED | Mouse detected/connected indicator (ON = mouse OK) |
| GPIO 5 | LED | Error/no mouse indicator (ON = error/no mouse) |
| GPIO 13 | JOY_LEFT | Joystick Left input |
| GPIO 14 | JOY_RIGHT | Joystick Right input |
| GPIO 15 | POT_Y | Potentiometer Y-axis |
| GPIO 16 | JOY_UP | Joystick Up input (also used for RMB) |
| GPIO 17 | JOY_DOWN | Joystick Down input |
| GPIO 18 | JOY_BUTTON | Joystick Button input (used for LMB) |
